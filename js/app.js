...
function initSEO(){
    ...
    if($('meta-desc')) $('meta-desc').content = s.description;
    if($('meta-keys')) $('meta-keys').content = s.keywords;
    if($('canonical')) $('canonical').href = s.siteUrl;
    
    if($('og-title')) $('og-title').content = `${C.name} — ${C.role}`;
    if($('og-desc')) $('og-desc').content = s.description;
    if($('og-url')) $('og-url').content = s.siteUrl;
    if($('og-img')) $('og-img').content = s.ogImage;
    
    if($('tw-title')) $('tw-title').content = `${C.name} — ${C.role}`;
    if($('tw-desc')) $('tw-desc').content = s.description;
